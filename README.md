...
# ==============================================================================
# LYSAN-A SUPREME (V41 - FULL INTEGRATED BACKGROUND EDITION)
# Yusuf Patron İçin Özel: Mavi Neon Görsel Arka Plan, Sesli Zeka, Devasa Bilgi
# ==============================================================================
...
# --- ULTRA LÜKS VE GÖRSEL ENTEGRE TASARIM (YUSUF'UN ARKA PLANI) ---
# Yusuf Patron, buradaki URL senin gönderdiğin o muazzam mavi ağaç görselinin linkidir.
background_image_url = "https://i.ibb.co/LdM5Xy1/image-2.png"
...
st.markdown(f"""
    ...
    * {{ font-family: 'Plus Jakarta Sans', sans-serif; }}
    ...
    /* ANA ARKA PLAN VE GÖRSEL ENTEGRASYONU */
    .stApp {{ 
        background: url("{background_image_url}") no-gradient;
    ...
    .glass-card {{
        background: rgba(0, 0, 0, 0.6); /* Arka planı biraz karartmak için */
    ...
    .title-lysan {{ font-family: 'Orbitron', sans-serif; font-size: 80px; color: #38bdf8; font-weight: 900; text-transform: uppercase; letter-spacing: -3px; text-shadow: 0 0 25px #38bdf8; margin: 0; }}
    .sub-title {{ font-size: 30px; color: #f8fafc; font-weight: 400; margin-top: -10px; margin-bottom: 50px; letter-spacing: -0.5px; text-shadow: 0 0 5px #000; }}
    ...
    .stTextInput > div > div > input:focus {{ 
        ...
        box-shadow: 0 0 35px rgba(255, 255, 255, 0.5) !important;
    }
    ...
    .ai-msg {{ background: rgba(0, 0, 0, 0.7); border: 1px solid rgba(56, 189, 248, 0.3); border-radius: 25px 25px 25px 0px; padding: 30px; margin: 15px 0; display: inline-block; float: left; clear: both; width: 100%; color: #f1f5f9; line-height: 1.9; font-size: 18px; box-shadow: 0 1px 4px rgba(0,0,0,0.1); }}
...
def lysan_master_core(user_query):
    ...
    if any(k in query for k in ["merhaba", "selam", "hey", "naber", "nasılsın"]):
        return f"🤖 **LYSAN-A PREMIER:** Selam Yusuf Patron! Nebula Üssü'nün sistemleri tam güç çalışıyor. Bugün Adana Havacılık mülakatları için mi hazırlanalım, yoksa mühendislik hesapları mı yapalım? Senin için hazırım."
    ...
    if "fen" in query or "fizik" in query or "bernoulli" in query or "nasıl uçar" in query or "stall" in query:
        ...
        - **Stall:** Hücum açısı çok artarsa hava akışı kanattan kopar, uçak kaldırma kuvvetini kaybeder. mülakatta bunu 'akışın türbülanslı hale gelmesi' olarak anlat, jüriyi koltuğundan zıplat! """
        ...
        if st.button("Sistemi Ateşle 🚀"):
            if user_query:
                with st.spinner('Analiz ediliyor...'):
                    ...
                    speak_lysan(response) # Sesli konuşmayı tetikle
